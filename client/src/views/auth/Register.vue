...
const handleSubmit = async () => {
  const { success } = await restler.register(form);
  if (success) {
    router.push({
      name: "chat",
    });
    ...
    >
      ...
      >
        ...
        <p class="flex flex-col items-center justify-center mt-10 text-center">
          ...
          <router-link :to="{ name: 'auth-login' }" class="underline"
            >Sign in!</router-link
          >
        </p>
      ...
      <div class="p-5 bg-brand-grey-2 md:flex-1">
        <h3 class="my-4 text-2xl font-semibold text-white">Account Sign up</h3>
